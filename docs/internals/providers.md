...
| `acpRegistry` | [`Drivers/AcpRegistryDriver.ts`][acp]   |
...
Provider snapshots may also carry normalized subscription usage windows. The provider layer owns
polling and normalization because limits belong to one configured provider instance and its
credentials. The client only selects the snapshots relevant to a thread: its current provider first,
then root providers used earlier in that thread. These snapshots use the existing server-config
stream, so local, remote, and tunnel clients observe the same values.

Codex and Claude normalize subscription limits today. Cursor, Grok, OpenCode, and ACP registry
providers report usage as unsupported because their current adapters expose no equivalent account
limit endpoint.
...
[acp]: ../../apps/server/src/provider/Drivers/AcpRegistryDriver.ts
