...
var range=selection.getRangeAt(0).cloneRange();function math(node){return (node.nodeType===1?node:node.parentElement).closest('.equation');}
...
var fragment=document.createElement('div');fragment.appendChild(range.cloneContents());fragment.querySelectorAll('.actions,.source,.file-actions,.file-menu').forEach(function(node){node.remove();});fragment.querySelectorAll('.equation').forEach(function(node){node.replaceWith(document.createTextNode(node.getAttribute('data-source')));});
