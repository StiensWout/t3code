...
export const make = Effect.gen(function* () {
  ...
  const needsExtraction = serverRoot.endsWith(".asar");
  ...
  const partialDir = join(treeRoot, `${version}${PARTIAL_SUFFIX}`);
  ...
  const extract = Effect.gen(function* () {
    ...
    yield* fs.remove(partialDir, { recursive: true }).pipe(Effect.ignore);
    ...
    // The marker is written before the rename, so a directory named after the
    // version is complete by construction; a crash mid-extraction leaves only
    // a .partial directory that the next attempt removes.
